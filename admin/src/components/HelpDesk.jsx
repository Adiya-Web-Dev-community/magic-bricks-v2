...
const HelpDesk = () => {
  ...
  return (
    <main className="h-[calc(100vh-64px)] bg-cyan-200 px-5 lg:px-10 py-5 text-slate-700">
      <main className="h-full w-full flex rounded-xl overflow-hidden bg-white shadow">
        <main
          className={`group border-r duration-100 ${
            width < 1024 ? (selectUser?._id ? "w-0" : "w-full") : "w-[30%]"
          }`}
        >
          <section className="h-14 bg-slate-100 border-b flex items-center px-[18px] space-x-2.5">
          ...
          <section className="h-[calc(100%-56px)] overflow-y-scroll scrollbar scrollbar-w-1 scrollbar-thumb-transparent group-hover:scrollbar-thumb-slate-300 scrollbar-thumb-rounded-full">
            {isLoading
              ? Array(10)
                  .fill(0)
                  .map((_, j) => (
        ...
        {selectUser?._id ? (
          <main
            ...
            className={`group relative bg-[url(https://web.telegram.org/a/chat-bg-pattern-light.ee148af944f6580293ae.png)] bg-center duration-100 ${
              width < 1024 ? (selectUser?._id ? "w-full" : "w-0") : "w-[70%]"
            }`}
          >
            <main className="w-full h-full bg-white/75">
              <section className="h-14 bg-slate-100 border-b flex items-center px-[18px] space-x-1.5">
                ...
                <div
                  style={{
                    backgroundImage: `url(${selectUser.profileImg})`,
                  }}
          ...
          <main
            style={{ backgroundSize: "500px auto" }}
            className="w-0 lg:w-[70%] bg-[url(https://web.telegram.org/a/chat-bg-pattern-light.ee148af944f6580293ae.png)] bg-center"
          >
